...
namespace Esoteric.Finance.Data.Repositories
{
        ...
        where TContext : DbContext
    {
        ...
        public virtual async Task<IList<T>> GetAuditedEntities<T>(Expression<Func<T, bool>>? filter, CancellationToken cancellationToken)
            where T : CommonAuditedEntity
        {
            ...
            return await _context.Set<T>().AsQueryable()
                .Where(filter)
                .ToListAsync(cancellationToken);
        }

        public virtual async Task<IList<T>> GetNamedEntities<T>(Expression<Func<T, bool>>? filter, CancellationToken cancellationToken)
